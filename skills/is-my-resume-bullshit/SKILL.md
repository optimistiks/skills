...
**Fabricated / unsourceable claim** *(highest)* — a number with no dashboard, billing console, or record behind it ("10k+ users," "hundreds of manuscripts daily," "thousands of postings"), *or* an unprovable superlative or universal negative ("first to," "the only X that," "no existing implementation handled it," "best-in-class"). The tell: you couldn't produce the source if asked, or you'd have had to survey everyone to know it's true. Fix: replace a number with one you *can* source ("$X MRR at time of departure," "~N monthly reads from the console"), or cut it and describe the work; narrow an unbounded claim to what you actually verified ("no *public* implementation we found handled it"). A soft number — or an unbounded claim — kept is worse than none.
